...
const InnovaRental = () => {
  ...
  const { innova } = useParams();
  const city =
    innova?.replace("innova-", "").charAt(0).toUpperCase() +
      innova?.replace("innova-", "").slice(1) || "City";
  ...
  const faqs = [
    {
      q: `Can I book an Innova for ${city} Airport pickup or drop?`,
      a: `Yes, we provide Innova Crysta and Hycross rentals for the airport in ${city}. Our drivers ensure timely airport pickups and drops with complete comfort and reliability.`,
    },
    {
      q: `Do you offer Innova rental for sightseeing in ${city}?`,
      a: `Absolutely! You can hire an Innova for local sightseeing across ${city}, covering popular landmarks and attractions in and around the city.`,
    },
    {
      q: `Is Innova available for outstation trips from ${city}?`,
      a: `Yes. We provide Innova car rentals for outstation travel from ${city} to nearby destinations with one-way or round-trip travel options.`,
    },
    {
    ...
    {
      q: `How can I book an Innova cab in ${city}?`,
  ...
  return (
    <div className="innova-page">
      <head>
        ...
        <meta
          property="og:url"
          content={`https://www.cozycabz.com/${city.toLowerCase()}`}
        />
      </head>
      ...
      <section className="innova-content-section">
        ...
        <div className="innova-highlight-box">
          <p>
            {` ${city}, one of India’s major cities, offers a unique mix of
        ...
        <p>
          {`The Toyota Innova on rent in ${city} is known for its spacious
        ...
        <p>
          {`In ${city}, busy roads, booking an Innova with driver in Ahmedabad
